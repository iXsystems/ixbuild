...
sed -i '' "s|/etc/install.sh|/etc/install.sh ${VMDISK};/sbin/mount -t zfs freenas-boot/ROOT/default;/rescue/chroot /mnt sqlite3 /data/freenas-v1.db 'update system_advanced set adv_serialconsole = 1';/sbin/umount /mnt;/sbin/halt -p|g" uzipdir/etc/rc
if [ $? -ne 0 ] ; then exit 1; fi
